...
- Per-file `chroma_key` PRESET override (green/blue/weak/strong): each media file can use its own chroma key independent of the global preset; `chroma: false` per-file still disables it. Implemented in `dyst/config.py::chroma_preset_cfg`, parsed in `dyst/media.py::_validate_settings`, applied in `main.py::_spawn_overlay`. Tested in `scripts/test_chroma.py` (per-file chroma parsing + preset).
**Verified (commands + results):**

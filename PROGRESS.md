...
- **Overall:** Phase 0 ✅ · Phase 1a ✅ · Phase 1 core ✅ (tray wiring pending Phase 6)
- **Last updated:** Phase 1a completion (ticker + overlay fixes)
...
| 2 | Media scanning & pairing | ⬜ not started | validation, sidecar audio |
...
- **No sidecar audio yet** (Phase 5), and images remain silent.
- **AV1 hardware decode unsupported on this machine** → QtMultimedia gives audio-only.
  ...
  Sidecar audio (Phase 5) will restore sound for AV1 files.
...
2. **Phase 2 — media scanning & pairing:** file validation (Pillow/OpenCV probe), skip
   corrupt files with warnings, sidecar audio pairing — before chroma work.
3. **Phase 3 — chroma key:** the next big visual milestone (make the green-bg video
   actually transparent). Recommended after P2.
4. Phase 4 — overlay polish (GIF/APNG frames, monitor pick); Phase 5 — manager + audio;
